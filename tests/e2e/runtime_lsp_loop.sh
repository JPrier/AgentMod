...
run_root=$(mktemp -d "${TMPDIR:-/tmp}/agentmod-lsp-e2e.XXXXXX")
mkdir -p "$run_root/workspace/src"
printf '%s\n' '[package]' 'name = "fixture"' 'version = "0.1.0"' > \
    "$run_root/workspace/Cargo.toml"
printf '%s\n' 'pub fn fixture() {}' > "$run_root/workspace/src/lib.rs"
...
export AGENTMOD_LSP_HOST_PROGRAM="$repository/target/debug/agentmod-lsp-host"
unset AGENTMOD_LSP_SERVERS_JSON || true
...
turn=$("$repository/target/debug/agentmod" run "detect LSP project root" \
    --session "$session_id" --option 'mock_scenario="lsp_project_root"' --json)
...
grep -F '"root":' "$journal" >/dev/null

echo "runtime/CLI/harness/LSP tool-loop E2E passed"
