...
try {
    ...
    $webHost = (Resolve-Path "target\debug\agentmod-web-host.exe").Path
    $cli = (Resolve-Path "target\debug\agentmod.exe").Path
    $runRoot = Join-Path ([System.IO.Path]::GetTempPath()) (
        "agentmod-web-e2e-" + [guid]::NewGuid().ToString("N")
    )
    $workspace = Join-Path $runRoot "workspace"
    New-Item -ItemType Directory -Path $workspace -Force | Out-Null

    $env:AGENTMOD_RUNTIME_ENDPOINT = (
        "\\.\pipe\agentmod-web-e2e-" + [guid]::NewGuid().ToString("N")
    )
    $env:AGENTMOD_RUNTIME_AUTH_TOKEN = (
        "0123456789abcdef0123456789abcdef0123456789abcdef"
    )
    $env:AGENTMOD_HARNESS_PROGRAM = $harness
    $env:AGENTMOD_WEB_HOST_PROGRAM = $webHost
    ...
    try {
        ...
        $turn = & $cli run "search through the native Web host" `
            --session $created.session_id `
            --option 'mock_scenario="web_search"' --json | ConvertFrom-Json
        if ($LASTEXITCODE -ne 0) { throw "Web tool turn failed" }
        ...
        foreach ($required in @(
            ...
            if (@($events | Where-Object {
                ...
                throw "missing or duplicated Web lifecycle event: $required"
            }
        }
        ...
        if ($null -eq $projection -or $projection.provider -ne "mock" -or
            @($projection.results).Count -ne 0) {
            $details = $resultEntry | ConvertTo-Json -Depth 20
            throw "Web search projection was incorrect`n$details"
        }
        $artifactDirectory = Join-Path $runRoot (
            "sessions\" + $created.session_id + "\artifacts\web"
        )
        if (-not (Test-Path -LiteralPath $artifactDirectory)) {
            throw "Web host state was not session-scoped"
        }
        Write-Output "runtime/CLI/harness/Web tool-loop E2E passed"
    }
    finally {
