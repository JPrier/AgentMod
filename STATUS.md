...
| ACP frontend vertical slice | End-to-end validated | Independent `service → logic → data → dependency` crates use the official `agent-client-protocol` 2.0 SDK and stable ACP wire version 1. Capacity-one layer-owned streams preserve runtime credit-window backpressure and emit each `session/update` without turn buffering; dropped client streams issue runtime cancellation, and active cancellation is registered before the endpoint spawns response work. The stdio adapter negotiates capabilities, creates and loads runtime sessions, accepts text/resource-link prompts, requests client permission for durable continuations, and maps cancelled permission outcomes to ACP `cancelled`. `runtime_acp.ps1` uses 750 ms provider pacing to prove a real Windows update arrives at least 500 ms before prompt completion and commits one canonical provider completion; Unix automation contains the equivalent assertion. Rich content, per-session MCP declarations, and complete permission/cancellation E2Es remain pending |
...
1. Complete ACP permission/cancellation parity tests and rich content.
