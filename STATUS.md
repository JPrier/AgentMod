...
Session-style refocus, Phase 7 — product surfaces and introspection. The
...
writes, plugin memory/compaction/context transforms, concurrent child dispatch,
...
| Terminal frontend vertical slice | End-to-end validated | Independent Ratatui `service → logic → data → dependency` crates use only the runtime protocol; authenticated bootstrap, lazy session listing, canonical-history paging, multiline UTF-8 editor/history, session switching, Chat/Events/Context/Graph/Styles/Harnesses/Help views, live style details/diagnostics, committed streams with credit acknowledgements, cancellation, permission modal, token accounting, atomic `/branch <sequence> [style]`, runtime-advertised `/memory` and `/compaction` selectors, SDK-validated `/budget` selection, and terminal restoration are implemented. Rendering and logic tests pass; `runtime_tui_smoke.ps1` proves health/session/style-inspection mapping, one canonically committed provider stream, a deliberately restyled branch with an unchanged parent, and CLI/TUI component/budget-selected immutable bindings through the real Windows named pipe. The Unix script is syntax-checked but not process-executed. Remaining management panels and rich content remain pending |
...
| Session-style SDK | Implemented and unit tested | Five built-ins, strict owned TOML/JSON manifests, STYLE001–STYLE031 validation, graph/pipeline/harness compilation, availability/budget checks, inspectable descriptors and compatibility-bound cache keys; 25 tests. SDK-owned transforms normalize selectable components and hard execution ceilings while preserving compiler validation. Enabled child policies now select an exact child style, workspace mode, inheritance, context/token/cost budgets, tools, memory access, join/cancellation semantics, and reviewer bound. `persistent-chat@1.1.0` and `planner-worker@1.1.0` carry that complete policy; exact old bindings remain unavailable rather than being silently upgraded. `research-loop@1.1.0` declares the live bounded fresh-context/model/tool/artifact/loop graph, and `declarative-graph@1.1.0` supplies the live branch/approval/tool/bounded-loop fixture |
| Runtime session-style registry and binding | End-to-end validated on Windows | Runtime dependency discovers bounded user/project/plugin TOML/JSON sources and disable markers and persists compiled cache records; data compiles through the SDK and owns catalog/cache records; logic owns exact ID/version selection, compatibility, immutable binding, component/budget override compilation, and fail-closed restart validation; service exposes style/component list, inspect, validate, and compile plus binding creation/branch operations. Complete identity, manifest, compiled descriptor, memory/compaction/tool/harness/budget/permission selections are canonical and atomic in schema-v2 session metadata, `style.json`, and `style.lock`. CLI commands, the TUI Styles/component/budget view, and the TUI branch command select live styles and session configuration. `runtime_style_registry.ps1` proves two base bindings plus a distinct SQLite/sliding/budget binding, post-restart compatibility/execution, branch restyling, and no fallback after disablement; `runtime_tui_smoke.ps1` proves the four-layer TUI branch and component/budget-selection paths. Unix equivalents have been syntax-checked but not process-executed |
...
| Style-selected context, memory, and compaction | End-to-end validated for no/file/SQLite memory and none/sliding/tool-eviction selection on Windows | The SDK compiles retrieval timing, query construction, write policy, injection location, reserved context tokens, projection limits, and typed preservation requirements with fail-safe schema-v1 defaults. SDK-owned per-session transforms normalize client-selected providers/strategies and hard execution ceilings; runtime data recompiles them into new content-addressed bindings, and restart reapplies only retained selections before exact comparison. Runtime data routes no-memory, checksum-protected file memory, and SQLite FTS through distinct selected dependencies with session isolation plus item, query-byte, contribution-byte, projection-token, and hard serialized-byte bounds. Context construction, replacement, and compaction proposals traverse the existing style/plugin/user/mandatory pipeline; canonical boundary/phase events enforce exact memory-before-compaction ordering, bind retries to provider/model/options/current-input identity, recompute projection measurements during replay, recover completed phases exactly once, and fail closed after an ambiguous interceptor start. Canonical replacements preserve full conversation history and complete provenance. `runtime_style_context.ps1` proves no/file/SQLite style selection and none-vs-sliding execution; `runtime_style_registry.ps1` and `runtime_tui_smoke.ps1` prove CLI/TUI component/budget overrides, distinct binding hashes, restart compatibility, and post-restart execution. Summary/artifact handoff and automatic writes remain incomplete; Unix scripts have only been syntax-checked |
| Runtime harness registry and per-session selection | End-to-end validated on Windows | Runtime dependency owns an injected adapter registry and cancellation route, data normalizes catalog records, logic computes capability hashes and validates exact style requirements, and service exposes list/inspect plus create-time selection. Immutable style bindings, model proposals, action digests, grants, and canonical model-request events retain harness identity. Native and independently supervised deterministic fixture entries prove routing is not hard-coded; the fixture intentionally lacks image support for negative compatibility coverage. CLI supports `harness list`, `harness inspect`, and `session create --harness`; the command-driven TUI catalog selects a harness during `/new`. `runtime_harness_selection.ps1` proves discovery, compatible native/fixture turns, incompatible creation rejection, durable identity, canonical audit, restart validation, and post-restart execution. The Unix equivalent passes syntax validation but has not been process-executed. The fixture uses the deterministic native harness executable by default through a separate adapter/process configuration; complete third-party adapters remain planned. |
| Runtime style/graph introspection | End-to-end validated on Windows for replay/restart projection | Runtime logic builds a bounded inspection projection solely from the immutable binding and canonical replay state; service adds it to session inspect/replay without invoking effects. It exposes style/source/cache and harness identity, complete compiled nodes/edges, control/active/previous nodes, known next transitions plus conditional candidates, loop/retry counters, exact remaining step/token/iteration budgets, tool/permission/retry/termination configuration, blocking pipeline order and invocation history, memory provenance, compaction boundaries, child executions/joins/reviewer findings, and termination. Cost and elapsed-duration remainder are explicitly unavailable because canonical accounting does not yet retain them. The TUI fetches this projection on selection and material lifecycle events and renders a Graph view. The extended Windows research-loop E2E proves three-iteration projection, restart, and pure replay; the TUI smoke E2E still passes. Unix research automation is syntax-checked but not process-executed. Conditional transition evaluation remains conservative when its runtime variable environment is not canonical. |
| Runtime local RPC transport | Integration tested | Protocol 2.4 provides bounded framed negotiation, mandatory bootstrap-token authentication, concurrent local socket/named-pipe connections, request dispatch, style/harness/component discovery, optional session component and budget selections, ordered `StreamItem`/`StreamEnd` frames, committed-sequence binding, and bounded channel backpressure tests. A terminal-only style turn is explicitly framed as `StreamEnd` even when it emits zero provider events |
...
1. Complete style/graph/pipeline/child introspection and the remaining CLI/TUI
   selection and configuration surfaces.
2. Finish planner-worker result artifacts, workspace enforcement, concurrency,
   and crash-cut recovery coverage.
3. Finish summary/artifact/write context paths and plugin-provided context
   components.
