...
run_root=$(mktemp -d "${TMPDIR:-/tmp}/agentmod-mcp-e2e.XXXXXX")
...
turn=$("$repository/target/debug/agentmod" run "list MCP servers" \
    --session "$session_id" --option 'mock_scenario="mcp_server_list"' --json)
...
grep -F '"servers":[]' "$journal" >/dev/null

echo "runtime/CLI/harness/MCP tool-loop E2E passed"
