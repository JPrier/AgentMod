...
| ACP frontend vertical slice | End-to-end validated | Independent `service → logic → data → dependency` crates use the official `agent-client-protocol` 2.0 SDK and stable ACP wire version 1. The stdio adapter negotiates capabilities, creates and loads runtime sessions, accepts text/resource-link prompts, emits `session/update`, requests client permission for durable runtime continuations, and routes cancellation into runtime. `runtime_acp.ps1` proves initialize → session/new → session/prompt → update → end_turn against the real Windows daemon and one canonical provider completion; Unix automation is present. Low-latency forwarding, rich content, per-session MCP declarations, and complete permission/cancellation E2Es remain pending |
...
| Durable scheduling | End-to-end validated | A separate five-crate worker and dedicated protocol implement authenticated negotiation, checksum-protected one-time/recurring/runtime-event/process-output schedules, portable atomic replacement recovery, complete execution policy, deterministic create-once claims, idempotent terminal markers, corruption rejection and restart deduplication. Runtime and CLI have layer-local management/claim/execute mappings; the daemon automatically polls with bounded skipped-tick semantics, commits `scheduler.fired`, enters the normal intercepted turn path, marks terminal only after canonical completion, and does not repeat after restart. `scheduler_worker.ps1` and `runtime_scheduler.ps1` pass; committed event/output delivery, continuation wakeups, and TUI management remain pending |
...
- Capability-host survival or reconnectable transport for live inherited-handle reattachment after a host crash.
...
- Committed runtime-event/process-output schedule delivery, continuation
  wakeups, and schedule TUI management.
...
1. Complete low-latency ACP forwarding and permission/cancellation parity tests.
2. Add scheduled runtime-event/output delivery, then MCP OAuth/restart-persistent cursors and remaining TUI management panels.
3. Expand acceptance automation beyond the current coding loop and kernel/in-process evidence.
