...
creation now select runtime-advertised memory and compaction components; the
...
| Session-style SDK | Implemented and unit tested | Five built-ins, strict owned TOML/JSON manifests, STYLE001–STYLE031 validation, graph/pipeline/harness compilation, availability/budget checks, inspectable descriptors and compatibility-bound cache keys; 23 tests. Enabled child policies now select an exact child style, workspace mode, inheritance, context/token/cost budgets, tools, memory access, join/cancellation semantics, and reviewer bound. `persistent-chat@1.1.0` and `planner-worker@1.1.0` carry that complete policy; exact old bindings remain unavailable rather than being silently upgraded. `research-loop@1.1.0` declares the live bounded fresh-context/model/tool/artifact/loop graph, and `declarative-graph@1.1.0` supplies the live branch/approval/tool/bounded-loop fixture |
| Runtime session-style registry and binding | End-to-end validated on Windows | Runtime dependency discovers bounded user/project/plugin TOML/JSON sources and disable markers and persists compiled cache records; data compiles through the SDK and owns catalog/cache records; logic owns exact ID/version selection, compatibility, immutable binding, component override compilation, and fail-closed restart validation; service exposes style/component list, inspect, validate, and compile plus binding creation/branch operations. Complete identity, manifest, compiled descriptor, memory/compaction/tool/harness/budget/permission selections are canonical and atomic in schema-v2 session metadata, `style.json`, and `style.lock`. CLI commands, the TUI Styles/component view, and the TUI branch command select live styles and components. `runtime_style_registry.ps1` proves two base bindings plus a distinct SQLite/sliding binding, post-restart compatibility/execution, branch restyling, and no fallback after disablement; `runtime_tui_smoke.ps1` proves the four-layer TUI branch and component-selection paths. Unix equivalents have been syntax-checked but not process-executed |
...
| Runtime local RPC transport | Integration tested | Protocol 2.3 provides bounded framed negotiation, mandatory bootstrap-token authentication, concurrent local socket/named-pipe connections, request dispatch, style/harness/component discovery, optional session component selections, ordered `StreamItem`/`StreamEnd` frames, committed-sequence binding, and bounded channel backpressure tests. A terminal-only style turn is explicitly framed as `StreamEnd` even when it emits zero provider events |
